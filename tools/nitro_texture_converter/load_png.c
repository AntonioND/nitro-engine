...
void *LoadPNGtoARGB(char *filename, int *buffer_size)
{
	...
	if ((PNG_width != 8 && PNG_width != 16 && PNG_width != 32
	     && PNG_width != 64 && PNG_width != 128 && PNG_width != 256
	     && PNG_width != 512 && PNG_width != 1024)
	    && (PNG_height != 8 && PNG_height != 16 && PNG_height != 32
		&& PNG_height != 64 && PNG_height != 128 && PNG_height != 256
		&& PNG_height != 512 && PNG_height != 1024)) {
	...
	if (PNG_color_type == PNG_COLOR_TYPE_PALETTE) {
		...
		if (PNG_depth < 8)	//Expand to 8 bits
	...
	*buffer_size = PNG_height * PNG_width * 4;
	...
	for (y = 0; y < PNG_height; y++) {
		for (x = 0; x < PNG_width; x++) {
			...
			IMAGE_BUFFER[(y * PNG_width) + x] =
			    (((unsigned int)alpha) << 24) | (((unsigned int)red)
							     << 16) |
