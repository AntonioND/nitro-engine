...
void * NE_Alloc(NEChunk * first_chunk, int size, int align)
{
	...
	if(align < 4) align = 4; //Minimun alignment...
	...
	while(1)
	{
		if(chunk_search->status == NE_STATE_FREE)
		{
			...
			else if( chunk_size > size) //If we have more space...
			{
				...
				if( ((int)free_chunk->start & (align-1)) == 0)
				{
					...
					NE_AssertPointer(new_chunk,"NE_Alloc: Couldn't allocte chunk. (1)");
					...
					if(next_chunk != NULL) //If this is not last chunk...
					{
						...
						//Shouldn't be free
						NE_Assert(next_chunk->status != NE_STATE_FREE,"NE_Alloc: Possible list corruption. (1)");
					}
					else
					{
						new_chunk->next = NULL;
				...
				else //If we need to align it, it is a bit more complicated...
				{
					//Check if even with disalignment we can create room for this
					...
					if(end_ptr < (int)free_chunk->end) //OK
					{
						...
						NE_AssertPointer(new_chunk,"NE_Alloc: Couldn't allocte chunk. (2)");
						NE_AssertPointer(new2_chunk,"NE_Alloc: Couldn't allocte chunk. (3)");
						...
						if(next_chunk != NULL) //If this is not last chunk...
						{
							...
							//Shouldn't be free
							NE_Assert(next_chunk->status != NE_STATE_FREE,"NE_Alloc: Possible list corruption. (2)");
						}
						else
						{
					...
					else if(end_ptr == (int)free_chunk->end) //OK. Even easier...
					{
						...
						if(next_chunk != NULL) //If this is not last chunk...
						{
							...
							//Shouldn't be free
							NE_Assert(next_chunk->status != NE_STATE_FREE,"NE_Alloc: Possible list corruption. (3)");
						}
						else
						{
							new_chunk->next = NULL;
		...
		if(chunk_search->next == NULL) return NULL; //No more chunks... Not enough free space.
...
void NE_Free(NEChunk * first_chunk, void * pointer)
{
	...
	while(1)
	{
		if(chunk_search->start == pointer) 
		{
			...
			if(chunk_search->status != NE_STATE_USED) return;
			...
			if(previous_chunk) if(previous_chunk->status == NE_STATE_FREE)
			{
				//We can join them
				//
				...
				//Check the next one
				if(next_chunk) if(next_chunk->status == NE_STATE_FREE)
				{
					//We can join them
					//
					...
					if(next_next_chunk)
					{
						...
						NE_Assert(next_next_chunk->status != NE_STATE_FREE,"NE_Free: Possible list corruption. (1)");
			...
			if(next_chunk) if(next_chunk->status == NE_STATE_FREE)
			{
				...
				if(next_next_chunk)
				{
					//Next Next should be used. If not, something bad happens here.
					NE_Assert(next_next_chunk->status != NE_STATE_FREE,"NE_Free: Possible list corruption. (2)");
		...
		if(chunk_search->next == NULL) return;
...
int NE_GetSize(NEChunk * first_chunk, void * pointer)
{
	...
	while(1)
	{
		...
		if(chunk_search->next == NULL) return 0;
...
void NE_MemGetInformation(NEChunk * first_chunk, NEMemInfo * info)
{
	...
	while(1)
	{
		...
		if(chunk_search->next == NULL) break;
		chunk_search = chunk_search->next;
	}
