...
#ifndef __NE_GENERAL_H__
#define __NE_GENERAL_H__
 ...
 *  \brief Updates input data for internal use. Use this if you are using NE_API or window system. 

You have to call scanKeys() each frame for this to work. */
 ...
 *  \param topscreen Pointer to a void function with the 3D scene of top sceen.
 *  \param downscreen Pointer to a void function with the 3D scene of lower sceen.  */
void NE_ProcessDual(NE_Voidfunc topscreen, NE_Voidfunc downscreen);

/*! \fn    void NE_InitConsole(void);
 *  \brief Inits libnds' console in  main screen. Works in dual 3D mode. It uses VRAM_F. */
 ...
 *  \brief Sets the part of screen which displays the 3D image and sets a new projection.
 *  \param x1 (x1,y1) Down-left pixel. 
 *  \param y1 (x1,y1) Down-left pixel.
 *  \param x2 (x2,y2) Up-right pixel.
 *  \param y2 (x2,y2) Up-right pixel.
 
In dual 3D mode it is reseted in NE_ProcessDual, so you have to call it inside your drawing function. 

After NE_2DViewInit() viewport is set to (0,0,255,191) for a while, in the next NE_Process() 
it will go back to what you set (in Dual 3D it goes to default values). 

You can't set another viewport for 2D objects with this function as it changes the 
projection matrix, use glViewport(). This isn't saved anywhere, you will have to use it after
NE_2DViewInit() every frame. */
 ...
 *  \param n Near plane. 
 *  \param f Far plane. */
#define NE_ClippingPlanesSet(n,f) NE_ClippingPlanesSetI(floattof32(n),floattof32(f))
...
//The compiler works OK with this	
#ifdef REG_BGOFFSETS
	#undef REG_BGOFFSETS 
#endif
...
Don't use it if the noise effect is paused, the effect won't be updated until it is unpaused. */
 ...
 *  \param shift Amplitude. Default is 9. If you want a bigger wave, use lower numbers. */
...
NE_CAN_SKIP_VBL allows the function skip the VBL wait if CPU load is greater than 100. You should use
this if you don't need to load textures or do anything else during VBL. You have to set NE_HBLFunc() 
to HBL interrupt function for this to work. */
void NE_WaitForVBL(NE_UPDATE_FLAGS flags);

/*! \fn    int NE_GetCPUPercent(void);
 *  \brief Returns CPU usage percent. 
 
You need to use NE_WaitForVBL() and set NE_HBLFunc() to HBL interrupt function for this to work. */
int NE_GetCPUPercent(void);

/*! \fn    bool NE_GPUIsRendering(void);
 *  \brief Returns false if VCOUNT is between 192 and 213. If not, true.
 
If true, you should't load textures (during the game). If you try to load textures, there is a
moment when the GPU can't access that data, so there will be glitches. */
bool NE_GPUIsRendering(void);

//------------------------------------------------------------------------------
#ifdef NE_DEBUG         //Debug
#define NE_AssertMinMax(min,value,max,format...) { if(((min)>(value))||((max)<(value))) { char string[256]; sprintf(string, ##format); __NE_debugprint(string); } }
#define NE_AssertPointer(ptr,format...) { if(!(ptr)) { char string[256]; sprintf(string, ##format); __NE_debugprint(string); } }
#define NE_Assert(cond,format...) { if(!(cond)) { char string[256]; sprintf(string, ##format); __NE_debugprint(string); } }
#define NE_DebugPrint(format...) { char string[256]; sprintf(string, ##format); __NE_debugprint(string); }
/*! \fn    void __NE_debugprint(const char * text);
 *  \brief Function used internally by Nitro Engine to report error messages when NE_DEBUG is defined.
 *  \param text Text to output. */
void __NE_debugprint(const char * text);

/*! \fn    void NE_DebugSetHandler(void (*fn)(const char*));
 *  \brief Sets the debug handler where Nitro Engine will send debug information.
 *  \param fn Handler function. */
void NE_DebugSetHandler(void (*fn)(const char*));

/*! \fn    void NE_DebugSetHandlerConsole(void);
 *  \brief Sets libnds's console as debug handler where Nitro Engine will send debug information. */
void NE_DebugSetHandlerConsole(void);
#endif
//------------------------------------------------------------------------------
#ifndef NE_DEBUG         //No debug
...
/*! @defgroup touch_test Touch test.
	...
	If you wanted to know if you are touching something, this is for you! These functions perform
	tests to know if an object is under the stylus coordinates and return its distance from the camera.
	
	Note: There is a bug when first two coordinates of viewport are different from (0,0) in gluPickMatrix.
	Note2: This uses last stylus coordinates even if it has been released. You will have to check
	    by yourself if it is really touching the screen.
	Note3: It two objects are overlaping the test may fail to diferenciate which of them is closer to the
	    camera.
	Note4: If you want to draw something without using NE_ModelDraw(), you will have to move the view
	    to the center of your model and use PosTest_Asynch(0,0,0).
	
	How to use this:
	1-Init a 'touch test mode' with NE_TouchTestStart() to prepare the hardware to perform this test.
      During this mode, polygons are not drawn. Because of that, you could (and should) use models with 
	  less details than originals (no texture, no normals, less polygons...).
	
	2-Call NE_TouchTestObject().
	
	3-Draw the model with NE_ModelDraw().
	
	4-Call NE_TouchTestResult() to know if it is being touched. If it is touched, it returns the distance
      from the camera. ;)
	
	5-Repeat 2-4 for each model you want to test.
	
	6-Call NE_TouchTestEnd() to return back to normal. Polygons are drawn on screen after this.
...
Polygons drawn after this won't be displayed on screen. It is a good idea to use models with less details
than the original to this test.
 ...
 *  \brief Returns -1 if the model is NOT being touched. If it is touched, returns the distance from
           the camera. Quite useful, isn't is? ;) */
int NE_TouchTestResult(void);

/*! \fn    void NE_TouchTestEnd(void);
 *  \brief Ends the 'touch test mode' and sets the matrices that were saved before.
