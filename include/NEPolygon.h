...
#ifndef __NE_POLYGON_H__
#define __NE_POLYGON_H__
 ...
 *  \param y (x,y,z) Vector of the light.
 *  \param z (x,y,z) Vector of the light.*/
#define NE_LightSet(n,c,x,y,z) NE_LightSetI(n,c,floattov10(x),floattov10(y),floattov10(z))
 ...
 *  \param y (x,y,z) Unit vector. 
 *  \param z (x,y,z) Unit vector. */
#define NE_PolyNormal(x,y,z) NE_PolyNormalI(floattov10(x),floattov10(y),floattov10(z))
 ...
 *  \param y (x,y,z) Vertex coordinates. 
 *  \param z (x,y,z) Vertex coordinates. */
#define NE_PolyVertex(x,y,z) NE_PolyVertexI(floattov16(x),floattov16(y),floattov16(z))
...
/*! \fn    void NE_PolyFormat(u8 alpha, u8 id, NE_LIGHT_ENUM lights, NE_CULLING_ENUM culling, NE_OTHER_FORMAT_ENUM other);
 *  \brief Enable/disable multiple options.
 *  \param alpha Alpha value. 0 = Wireframe, 31 = Opaque, 1-30 Transparent. You can only blend one polygon over another if they have different ID.
 *  \param id Polygon ID used for antialias, blending and outlining. (0 - 63)
 *  \param lights Lights used... Use NE_LIGHT_ENUM for this.
 *  \param culling What polygons must not be drawn. Possible options in NE_CULLING_ENUM.
 *  \param other Other parameters. Possible flags in NE_OTHER_FORMAT_ENUM. */
void NE_PolyFormat(u8 alpha, u8 id, NE_LIGHT_ENUM lights, NE_CULLING_ENUM culling, NE_OTHER_FORMAT_ENUM other);

/*! \fn    void NE_OutliningEnable(bool value);
 *  \brief Enable/disable outlining.
 *  \param value True/False for enabling/disabling.
 
You have to set some colors for this with NE_OutliningSetColor(). Color 0 works with polygon IDs
0 - 7, color 1 with IDs 8 - 15... 8 colors available. Only works with opaque or wireframe polygons.*/
...
What does this actually do? O.o I can only notice that if it is enabled colors are darker and 
some polygons are not iuminated...*/
...
It uses VRAM_C as color bitmap and VRAM_D as depth bitmap. You have to copy data there and
then use this function. Those 2 VRAM banks can't be used as texture banks with this enabled,
so you have to call NE_TextureSystemReset(0,0,USE_VRAM_AB); before using this.

Dual 3D mode needs those two banks for the display capture, so you can't use a clear BMP. 

VRAM_C: ABBBBBGGGGGRRRRR -- Alpha - Blue - Green - Red
VRAM_D: FDDDDDDDDDDDDDDD -- Fog - Depth (0 = near, 0x7FFF = far)

Use this only if you really need it. If you want to display just an image, set the rear plane
alpha to 0 with NE_ClearColorSet() and use a simple background. This is meant to be used only
if you need to set different depths to every pixel or enable the fog for just some pixels. */
...
/*! \fn    void NE_FogEnable(u8 shift, u32 color, u8 alpha, int mass, int depth);
 *  \brief Enables fog and sets its parameters.
 *  \param shift Distance between fog bands. (1 - 15)
 *  \param color Fog color.
 *  \param alpha Alpha value.
 *  \param mass Quantity of fog.
 *  \param depth Start point of fog. (0 - 7FFFh)  [Use float_to_12d3(n) int_to_12d3(n) ?]
