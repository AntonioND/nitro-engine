...
#ifndef __NE_CAMERA_H__
#define __NE_CAMERA_H__
...
/*! @defgroup camera_system Camera System.
	...
	Note: Be careful. If the 'look' vector of the camera is on the same axis as the 'up'
	vector, projection will be incorrect. For example, if 'up' = (0,1,0), avoid looking 
	at (0,1,0) and (0,-1,0) and very similar vectors (because of unaccuracy of math 
	functions).
	
	Note 2: This could use hardware accelerated ways to calculate, but I prefer this way
	to avoid getting an incorrect matrix and never being able to recuperate it without
	reseting the camera.
	
	Note 3: This is not meant to be your game's camera system. This is just for testing
	levels and things like that.
	@{
*/

/*! \struct NE_Camera
 *  \brief  Holds information of the camera. The matrix is only updated when it is needed. */
typedef struct {
	m4x4 matrix;
...
/*! \fn    void NE_CameraSetI(NE_Camera * cam, int xfrom, int yfrom, int zfrom, int xto, int yto, int zto, int xup, int yup, int zup);
...
/*! \def   NE_CameraSet(NE_Camera * cam, float xfrom, float yfrom, float zfrom, float xto, float yto, float zto, float xup, float yup, float zup);
 *  \brief Set coordinates for a NE_Camera struct.
 ...
 *  \param y (x,y,z) Vector.
 *  \param z (x,y,z) Vector. */
#define NE_CameraMove(c,x,y,z) NE_CameraMoveI(c,floattof32(x),floattof32(y),floattof32(z))

/*! \fn    void NE_CameraMoveFreeI(NE_Camera * cam, int front, int right, int up);
...
#define NE_CameraMoveFree(c,f,r,u) NE_CameraMoveFreeI(c,floattof32(f),floattof32(r),floattof32(u))
 ...
 *  \param y (x,y,z) Axis vector.
 *  \param z (x,y,z) Axis vector. */
#define NE_CameraRotateAxis(c,a,x,y,z) NE_CameraRotateAxisI(c,a,floattof32(x),floattof32(y),floattof32(z))
 ...
 *  \param numcameras Number of cameras. If it is less than 1, it will create space for NE_DEFAULT_CAMERAS.  */
 ...
 *  \param y (x,y,z) Vector to move the camera.
 *  \param z (x,y,z) Vector to move the camera. */
#define NE_ViewMove(x,y,z) NE_ViewMoveI(floattof32(x),floattof32(y),floattof32(z))
 ...
 *  \param y (x,y,z) Vector to scale the view by.  
 *  \param z (x,y,z) Vector to scale the view by.  */
#define NE_ViewScale(x,y,z) NE_ViewScaleI(floattof32(x),floattof32(y),floattof32(z))

/*! @} */ //camera_system

#endif //__NE_CAMERA_H__
