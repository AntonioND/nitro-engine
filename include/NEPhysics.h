...
/*! @defgroup physics Physics engine.
	...
	A very simple physics engine.
	
	YOU CAN ONLY USE BOUNDING BOXES!!!
...
/*! \enum NE_OnCollition
 *  \brief Possible actions that can be applied to an object after a collition. */
typedef enum {
...
typedef struct {
	...
	int oncollition; //What to do when a collition is detected.
	bool iscolliding; //true if a collition was detected last time.
	
	bool enabled;
	int physicsgroup; //to avoid useless collition checking
} NE_Physics;
...
#define NE_PhysicsSetRadius(p,r) NE_PhysicsSetRadiusI(p,floattof32(r))
...
#define NE_PhysicsSetGravity(p,g) NE_PhysicsSetGravityI(p,floattof32(g))
 ...
 *  \param p Pointer to the physics object. 
 *  \param f Friction. */
#define NE_PhysicsSetFriction(p,f) NE_PhysicsSetFrictionI(p,floattof32(f))
...
/*! \fn    void NE_PhysicsOnCollition(NE_Physics * physics, NE_OnCollition action);
 *  \brief Set action to do if collition.
 *  \param physics Pointer to the object.
 *  \param action Action. */
void NE_PhysicsOnCollition(NE_Physics * physics, NE_OnCollition action);

/*! \fn    bool NE_PhysicsIsColliding(NE_Physics * pointer);
 *  \brief Returns true if given object was colliding with other object last time it updated.
 *  \param pointer Pointer to the object. 
...
/*! \fn    bool NE_PhysicsCheckCollition(NE_Physics * pointer1, NE_Physics * pointer2);
 *  \brief Returns true if given objects are colliding without checking physics groups.
