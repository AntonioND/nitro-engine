...
/*! \fn    int NE_MaterialTexLoadFAT(NE_Material * tex, GL_TEXTURE_TYPE_ENUM type, int sizeX, int sizeY, int param, char * path); 
 *  \brief Load a texture from FAT and set it to a NE_Material struct. Returns 1 if OK, 0 if error. 
 *  \param tex Material. 
 *  \param type Texture type. 
 *  \param sizeX (sizeX, sizeY) Size of texture, Don't have to be power of 2 :).
 *  \param sizeY (sizeX, sizeY) Size of texture. Don't have to be power of 2 :). 
...
/*! \fn    int NE_MaterialTexLoad(NE_Material * tex, GL_TEXTURE_TYPE_ENUM type, int sizeX, int sizeY, int param, void * texture);
 *  \brief Load a texture from RAM and set it to a NE_Material struct. Returns 1 if OK, 0 if error. 
 *  \param tex Material. 
 *  \param type Texture type. 
 *  \param sizeX (sizeX, sizeY) Size of texture in pixels. Don't have to be power of 2 :).
 *  \param sizeY (sizeX, sizeY) Size of texture in pixels. Don't have to be power of 2 :). 
 *  \param param Parameters of the texture. 
 ...
 *  \brief Copies the texture of a material into another material. You can delete them as usual.
 *  \param source Source. 
 *  \param dest Destiny. */
...
If NULL, it will disable textures and new polygons won't be affected by them until it is called again. */
...
/*! \fn    void NE_TextureSystemReset(int texture_number, int palette_number, _NE_BANK_FLAGS_ bank_flags);
 *  \brief Resets texture and palette system.
 *  \param texture_number Max number of textures. If lower than 1, it will create 
 *         space for NE_DEFAULT_TEXTURES.
 *  \param palette_number Max number of palettes. If lower than 1, it will create 
 *         space for NE_DEFAULT_PALETTES. 
 *	\param bank_flags VRAM banks where Nitro Engine can allocate textures. If Dual 3D mode,
 *	       only A and B are available. If you are planning to use a depth BMP, you can only
 *	       use banks A and B. If you don't tell Nitro Engine to use at least a valid bank, it
...
/*! \fn    void NE_MaterialSetPropierties(NE_Material * tex, u32 diffuse, u32 ambient, u32 specular, u32 emission, bool vtxcolor, bool useshininess);
 *  \brief Sets propierties of this material.
 *  \param tex Material.
 *  \param diffuse Set diffuse color, lights that directly hits the polygon.
 *  \param ambient Set ambient color, lights that indirectly hits the polygon (walls reflections...).
 *  \param specular Set specular color, lights reflected towards the camera, like a mirror.
 ...
 *  \param useshininess If true, specular reflection will use the shininess table. */
void NE_MaterialSetPropierties(NE_Material * tex, u32 diffuse, u32 ambient, u32 specular, u32 emission, bool vtxcolor, bool useshininess);

/*! \fn    void NE_MaterialSetDefaultPropierties(u32 diffuse, u32 ambient, u32 specular, u32 emission, bool vtxcolor, bool useshininess);
 ...
 *  \param vtxcolor If true, diffuse reflection will work as color command.
 *  \param useshininess If true, specular reflection will use the shininess table. */
void NE_MaterialSetDefaultPropierties(u32 diffuse, u32 ambient, u32 specular, u32 emission, bool vtxcolor, bool useshininess);

/*! \fn    void * NE_TextureDrawingStart(NE_Material * tex);
 *  \brief Enables drawing for given texture. Returns a pointer to the data.
 *  \param tex Texture to draw.
 
Use this DURING VBL. YOU MUST USE NE_TextureDrawingEnd() WHEN YOU FINISH DRAWING. 
IF YOU DON'T, GPU WON'T BE ABLE TO RENDER ANY TEXTURE TO SCREEN. */
