...
*/
	while(size > 0)
		...
		if(colordetected)
			...
			unsigned int newcommand = (Command[3] << 24) + (Command[2] << 16) + (Command[1] << 8) + (Command[0]);
...
void Divide_Commands()
{
	...
	Arguments = Command_Arguments[0] + Command_Arguments[1] + Command_Arguments[2] + Command_Arguments[3];
}
...
while(size > 0)
	...
	printf("\nCommands:        %02x, %02x, %02x, %02x  (%08x)",Command[0],Command[1],Command[2],Command[3],*Pointer);
	...
	printf("\nArgument Number: %02d, %02d, %02d, %02d  (%d)\n",Command_Arguments[0],Command_Arguments[1],Command_Arguments[2],Command_Arguments[3],Arguments);
	...
	for(int i = 0; i < 4; i++)
		{
		if(Command[i] == Color_Command) printf("\nColor detected!\n");
