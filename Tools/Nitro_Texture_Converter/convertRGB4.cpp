...
{
	printf("RGB4: -If image alpha == 0 -> Color index = 0.\n");
	...
	for(c = 0; c < size; c+=16)
		...
		if(data_pointer[c+3] > 0)
			index_[0] = Palette_GetIndex((data_pointer[c]>>3) & 31,(data_pointer[c+1]>>3) & 31,(data_pointer[c+2]>>3) & 31);
		else index_[0] = 0; // 0 = transparent

		if(data_pointer[c+3] > 0)
			index_[1] = Palette_GetIndex((data_pointer[c+4]>>3) & 31,(data_pointer[c+4+1]>>3) & 31,(data_pointer[c+4+2]>>3) & 31);
		else index_[1] = 0;

		if(data_pointer[c+3] > 0)
			index_[2] = Palette_GetIndex((data_pointer[c+8]>>3) & 31,(data_pointer[c+8+1]>>3) & 31,(data_pointer[c+8+2]>>3) & 31);
		else index_[2] = 0;

		if(data_pointer[c+3] > 0)
			index_[3] = Palette_GetIndex((data_pointer[c+12]>>3) & 31,(data_pointer[c+12+1]>>3) & 31,(data_pointer[c+12+2]>>3) & 31);
		else index_[3] = 0;

		unsigned char save_to_file = (index_[0] & 0x3) | ((index_[1] & 0x3) << 2) | 
