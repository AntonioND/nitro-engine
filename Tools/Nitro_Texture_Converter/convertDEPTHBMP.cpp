...
{
	printf("DEPTHBMP: -Depth is calculated:\n");
	...
	//	int depth = ((data_pointer[a] + data_pointer[a+1] + data_pointer[a+2]) << 6 ) / 3;
		...
		if(data_pointer[a]) pixel_data = 0x00000;
		else pixel_data = 0x67FF + (0x1800 * data_pointer[a+2] / 255);

		pixel_data &= 0x7FFF;
		if(data_pointer[a+3] == 0) pixel_data |= 1 << 15; //If alpha == 0 -> Fog enabled

		if(fwrite(&pixel_data,sizeof(pixel_data),1,OUTPUT_FILE) != 1)
			{
			fclose(OUTPUT_FILE);
			printf("Write error!!\n\n");
			return -1;
