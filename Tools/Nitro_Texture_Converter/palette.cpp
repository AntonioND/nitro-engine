...
for(a = 0; a < colors_used; a++)
	...
	if(palette_ptr[a].red == red && palette_ptr[a].green == green &&
			palette_ptr[a].blue == blue) return;
	}

if(can_optimize) //Find if there is a very similar color...
	{
	for(a = 0; a < colors_used; a++) //Find the most similar color
		{
		if(MAX_COLOR_DIFFERENCE > Square(palette_ptr[a].red - red) + Square(palette_ptr[a].green - green) +
			Square(palette_ptr[a].blue - blue)) return; //OK ;)
		}
	}

if(colors_used < max_colors)//Add the color
	{
...
for(a = 0; a < colors_used; a++)
	...
	//If same color
	if(palette_ptr[a].red == red && palette_ptr[a].green == green &&
		palette_ptr[a].blue == blue) return a;
	}
...
if(can_optimize)
	...
	for(a = 0; a < colors_used; a++)
		{
		int newdifference = Square(palette_ptr[a].red - red) + Square(palette_ptr[a].green - green) +
